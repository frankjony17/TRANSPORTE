...
class CirculacionEventualRepository extends EntityRepository
{
    ...
    public function findVehiculosCirculacionOrdinaria($uo_id)
    {
        ...
        $query .= "(SELECT fecha_inicial FROM circulacion_eventual WHERE fecha_final >= '". date('d-m-Y') ."' AND hora_final >= '". date('H:i:s') ."' AND chofer_vehiculo_id = chofer_vehiculo.id) AS fecha_inicial, ";
        $query .= "(SELECT fecha_final FROM circulacion_eventual WHERE fecha_final >= '". date('d-m-Y') ."' AND hora_final >= '". date('H:i:s') ."' AND chofer_vehiculo_id = chofer_vehiculo.id) AS fecha_final, ";
        $query .= "(SELECT hora_inicial FROM circulacion_eventual WHERE fecha_final >= '". date('d-m-Y') ."' AND hora_final >= '". date('H:i:s') ."' AND chofer_vehiculo_id = chofer_vehiculo.id) AS hora_inicial, ";
        $query .= "(SELECT hora_final FROM circulacion_eventual WHERE fecha_final >= '". date('d-m-Y') ."' AND hora_final >= '". date('H:i:s') ."' AND chofer_vehiculo_id = chofer_vehiculo.id) AS hora_final, ";
        $query .= "(SELECT aprobado FROM circulacion_eventual WHERE fecha_final >= '". date('d-m-Y') ."' AND hora_final >= '". date('H:i:s') ."' AND chofer_vehiculo_id = chofer_vehiculo.id) AS aprobado, ";
        $query .= "(SELECT pendiente FROM circulacion_eventual WHERE fecha_final >= '". date('d-m-Y') ."' AND hora_final >= '". date('H:i:s') ."' AND chofer_vehiculo_id = chofer_vehiculo.id) AS pendiente, ";
        $query .= "(SELECT tarea FROM circulacion_eventual WHERE fecha_final >= '". date('d-m-Y') ."' AND hora_final >= '". date('H:i:s') ."' AND chofer_vehiculo_id = chofer_vehiculo.id) AS tarea, ";
        $query .= "(SELECT circulacion_eventual_tipo_id FROM circulacion_eventual WHERE fecha_final >= '". date('d-m-Y') ."' AND hora_final >= '". date('H:i:s') ."' AND chofer_vehiculo_id = chofer_vehiculo.id) AS circulacion_eventual_tipo, ";
        $query .= "(SELECT id FROM circulacion_eventual WHERE fecha_final >= '". date('d-m-Y') ."' AND hora_final >= '". date('H:i:s') ."' AND chofer_vehiculo_id = chofer_vehiculo.id) AS circulacion_eventual_id ";
        $query .= "FROM vehiculo ";
        ...
        foreach ($fetch as $value) {
            $circulacion_eventual_ordinaria[] = array(
                ...
                'fecha_final' => $value['fecha_final'],
              	'hora_inicial' => $this->getValue($value['hora_inicial']),
                'hora_final' => $this->getValue($value['hora_final']),
    ...
    /**
     * Listar todos los vehículos de la Unidad Organizativa actual que esten trabajando en la fecha actual y que tengan circulacion evental extraordinaria.
     * 
     * @param type $value
     * @param type $uo_id
     * 
     * @return Object
     */
    public function findVehiculosCirculacionExtraOrdinaria($uo_id)
    {
        $query = "SELECT matricula_id, matricula.chapa, trabajador.nombre_apellidos AS chofer, unidad_organizativa.nombre AS unidad_organizativa, chofer_vehiculo.id AS chofer_vehiculo_id, vehiculo.id AS vehiculo_id, marca.nombre AS marca, modelo.nombre AS modelo, vehiculo_tipo.nombre AS tipo_vehiculo, situacion_operativa.nombre AS situacion_operativa, chofer.licencia, ";
        ...
        $query .= "circulacion_eventual_tipo.nombre AS circulacion_eventual_tipo FROM vehiculo ";
        $query .= "INNER JOIN matricula ON matricula.id = vehiculo.matricula_id ";
        $query .= "INNER JOIN marca ON marca.id = vehiculo.marca_id ";
        $query .= "INNER JOIN modelo ON modelo.id = vehiculo.modelo_id ";
        $query .= "INNER JOIN vehiculo_tipo ON vehiculo_tipo.id = vehiculo.vehiculo_tipo_id ";
        ...
        $query .= "INNER JOIN situacion_operativa_vehiculo ON  situacion_operativa_vehiculo.vehiculo_id = vehiculo.id ";
        $query .= "INNER JOIN situacion_operativa ON  situacion_operativa.id = situacion_operativa_vehiculo.situacion_operativa_id ";
        $query .= "WHERE unidad_organizativa.id = '".$uo_id."' AND  situacion_operativa.nombre = ('trabajando') AND circulacion_eventual_tipo.nombre = ('extraordinaria') ";

        $fetch = $this->getEntityManager()->getConnection()->fetchAll($query);

        $circulacion_eventual_extraordinaria = array();
        ...
        return $circulacion_eventual_extraordinaria;
    }
