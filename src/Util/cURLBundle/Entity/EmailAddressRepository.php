...
class EmailAddressRepository extends EntityRepository
{
    ...
    public function findUsers($uo_id=\NULL)
    {
        ...
        $query = $this->getManager()->createQuery('SELECT ea FROM cURLBundle:EmailAddress ea JOIN ea.unidadOrganizativa uo WHERE uo.id = :id');
        ...
        foreach ($emailAddress as $ea)
        {
            if (count($users = $ea->getUsers()) > 0)
            {
                ...
                {
                    $trabajador = $this->getManager()->getRepository('NomencladorBundle:Trabajador')->find(
